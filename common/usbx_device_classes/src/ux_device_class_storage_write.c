...
ULONG                   done_length;
    ...
    /* BOT phase error checks must run before any sense-based failures so that a
       malformed CBW gets the correct BOT recovery behavior (BBB sections 6.7).  */
    ...
    if (storage -> ux_slave_class_storage_lun[lun].ux_slave_class_storage_medium_loaded_status == 0)
    {
        ...
        /* No data will be received: residue is the full requested length.  */
        storage -> ux_slave_class_storage_csw_residue = storage -> ux_slave_class_storage_host_length;

#if !defined(UX_DEVICE_STANDALONE)
        /* Stall the OUT endpoint per BBB; host will recover via reset.  */
        _ux_device_stack_endpoint_stall(endpoint_out);
#endif
    ...
    /* BOT phase error checks were already performed above.  */
