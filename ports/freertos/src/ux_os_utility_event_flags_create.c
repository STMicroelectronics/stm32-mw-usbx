...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _ux_os_utility_event_flags_create                   PORTABLE C      */
/*                                                           6.4.1        */
/*  AUTHOR                                                                */
/*                                                                        */
/*    STMicroelectronics                                                  */
...
UINT _ux_os_utility_event_flags_create(UX_EVENT_FLAGS_GROUP *group_ptr, CHAR *name)
{
  ...
  /* Check if the event group is already created */
  ...
  group_ptr->event_group_handle = xEventGroupCreate();

  if (group_ptr->event_group_handle == NULL)
  {
    /* Error trap. */
    _ux_system_error_handler(UX_SYSTEM_LEVEL_THREAD, UX_SYSTEM_CONTEXT_UTILITY, UX_EVENT_ERROR);
    return UX_EVENT_ERROR;
