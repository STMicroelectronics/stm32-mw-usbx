...
/*  CALLS                                                                 */
/*                                                                        */
/*    HAL_PCD_ClearEndpointStall            Clear STALL                   */
/*                                                                        */
/*  CALLED BY                                                             */
/*                                                                        */
/*    STM32 Controller Driver                                             */
...
UINT _ux_dcd_stm32_endpoint_reset(UX_DCD_STM32 *dcd_stm32, UX_SLAVE_ENDPOINT *endpoint)
{
  ...
  HAL_PCD_ClearEndpointStall(dcd_stm32 -> pcd_handle, endpoint -> ux_slave_endpoint_descriptor.bEndpointAddress);

  /* Flush buffer.  */
  HAL_PCD_FlushEndpoint(dcd_stm32 -> pcd_handle, endpoint->ux_slave_endpoint_descriptor.bEndpointAddress);

#ifndef UX_DEVICE_STANDALONE
  /* Wakeup pending thread. */
  if (_ux_device_semaphore_waiting(&endpoint -> ux_slave_endpoint_transfer_request.ux_slave_transfer_request_semaphore))
