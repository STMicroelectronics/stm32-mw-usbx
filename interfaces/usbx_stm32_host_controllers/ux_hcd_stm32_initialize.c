...
UINT _ux_hcd_stm32_initialize(UX_HCD *hcd)
{
  ...
  hcd_stm32 -> ux_hcd_stm32_ed_list = _ux_utility_memory_allocate(UX_NO_ALIGN, UX_REGULAR_MEMORY,
  ...
  /* Register USB HCD callacks. */
  _ux_hcd_stm32_register_callback(hcd_stm32, UX_NULL);

  /* Start USB Device.  */
  hcd -> ux_hcd_entry_function(hcd, UX_HCD_ENABLE_PORT, UX_NULL);
